...
/**
 * @file components/ui/HuileDetailModal.tsx
 * @description Full-screen huile (essential oil) detail overlay. Uses the exact
 * same design as the standalone HuileDetailClient page, but mounted on top of
 * the shop catalog so the catalog is never unmounted (scroll, pagination, and
 * shuffle order are preserved).
 *
 * History integration is identical to ProductDetailModal:
 *   - Parent calls pushState before rendering this component
 *   - Closing via ×/backdrop calls window.history.back()
 *   - Parent popstate listener calls onClose
 */
...
import {
  Share2, Heart, ShoppingBag, Check, ChevronRight,
  Minus, Plus, Droplets, Leaf, FlaskConical,
  Info, Sparkles, Award, ArrowLeft, X,
} from 'lucide-react';
...
export function HuileDetailModal({ productId, onClose, onRelatedHuileClick }: HuileDetailModalProps) {
  ...
  // Multi-format selection: variantId → quantity
  ...
  const toggleVariant = useCallback((variant: ProduitFiniEssence) => {
    if (variant.stock_disponible <= 0) return;
    setSelectedQuantities(prev => {
      const next = { ...prev };
      if (next[variant.id]) { delete next[variant.id]; } else { next[variant.id] = 1; }
      return next;
    });
  }, []);

  const updateVariantQuantity = useCallback((variant: ProduitFiniEssence, delta: number) => {
    setSelectedQuantities(prev => {
      const current = prev[variant.id] ?? 0;
      const nextQty = Math.max(1, Math.min(variant.stock_disponible, current + delta));
      return { ...prev, [variant.id]: nextQty };
    });
  }, []);

  const setVariantQuantityDirect = useCallback((variant: ProduitFiniEssence, qty: number) => {
    setSelectedQuantities(prev => {
  ...
  const selectedItems = useMemo(
    () => variants.filter(v => (selectedQuantities[v.id] ?? 0) > 0).map(v => ({ variant: v, quantity: selectedQuantities[v.id] })),
    [variants, selectedQuantities]
  );

  const totalQuantity = useMemo(() => selectedItems.reduce((s, i) => s + i.quantity, 0), [selectedItems]);
  const totalPrice    = useMemo(() => selectedItems.reduce((s, i) => s + i.variant.prix_actuel * i.quantity, 0), [selectedItems]);
  ...
  const handleAddToCart = async () => {
    ...
    for (const item of selectedItems) {
      ...
      try {
        ...
        trackAddToCart({ id: String(variant.id), name: cartProduct.name, price: variant.prix_actuel, category: 'Huile', quantity });
      } catch {}
    }
  ...
  const handleShare = async () => {
    ...
    const result = await sharePage(
      ...
      product.name,
      isEn ? `Discover pure oil ${product.name} on Accessories Exclusif` : `Découvrez l'huile pure ${product.name} sur Accessoires Exclusifs`
    );
  ...
  return (
    <>
      ...
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={product?.name ?? 'Huile essentielle'}
      >
        {/* Back button */}
        <button
          onClick={() => window.history.back()}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            {!loading && !product && (
              <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                ...
                <button onClick={() => window.history.back()} className="text-gold underline text-sm">
                  {isEn ? 'Go back' : 'Retour'}
                </button>
              </div>
            )}
            ...
            {!loading && product && (
              <>
                {/* Breadcrumb */}
                <nav className="mt-2 mb-8 flex items-center gap-2 text-[11px] text-foreground/40 uppercase tracking-widest overflow-x-auto whitespace-nowrap scrollbar-hide">
                  <span className="hover:text-gold transition-colors cursor-pointer" onClick={() => window.history.back()}>
                    {isEn ? 'Shop' : 'Boutique'}
                  </span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 pt-8">
                    ...
                    <div className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.02] p-3.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-foreground capitalize">Pure Oil</p>
                      </div>
                    </div>
                  </div>
                </div>
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div
                                  ...
                                  className={cn('flex items-center gap-3.5 flex-1 min-w-0', isOutOfStock ? 'cursor-not-allowed' : 'cursor-pointer')}
                                >
                                  <div className={cn(
                                    'w-6 h-6 rounded-lg border flex items-center justify-center shrink-0 transition-colors',
                                    isSelected ? 'border-gold bg-gold text-black' : 'border-white/20 hover:border-white/40'
                                  )}>
                                    {isSelected && <Check size={14} strokeWidth={3} />}
                                  </div>
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span className="text-base font-bold text-foreground font-mono">Flacon {v.taille_ml} ml</span>
                                      {hasReduction && (
                                        <span className="bg-red-500/10 border border-red-500/20 text-red-400 text-[9px] font-bold px-1.5 py-0.5 uppercase tracking-wide rounded-md">Promo</span>
                                      )}
                                    </div>
                                ...
                                <div className="text-right shrink-0">
                                  {hasReduction ? (
                                    <div className="flex flex-col items-end">
                                      <span className="text-xs line-through text-foreground/40 font-mono">{formatPrice(originalPriceNum)}</span>
                                      <span className="text-base font-black text-gold font-mono">{formatPrice(v.prix_actuel)}</span>
                                    </div>
                                  ) : (
                              ...
                              {/* Qty stepper when selected */}
                              {isSelected && (
                                <div className="pt-3 border-t border-white/10 flex items-center justify-between">
                                  <span className="text-xs font-medium text-foreground/60">Quantité :</span>
                      ...
                      {selectedItems.length > 0 && (
                        <div className="mt-6 rounded-2xl border border-gold/30 bg-gold/10 p-5 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-5">
                          <div className="space-y-1 w-full sm:w-auto text-left">
                            <p className="text-xs font-bold uppercase tracking-widest text-foreground/70">Total de votre commande</p>
                            <p className="text-xs text-foreground/50">{selectedItems.map(it => `${it.quantity}× ${it.variant.taille_ml}ml`).join(' + ')}</p>
                            <p className="text-2xl font-black text-gold font-mono pt-1">{formatPrice(totalPrice)}</p>
                          </div>
                          <button
                            type="button"
                  ...
                  <div className="mb-8 flex overflow-x-auto border-b border-foreground/10 scrollbar-hide">
                    {(
                      ...
                      >
                        {tab.label}
                        {activeTab === tab.key && (
                          <motion.div layoutId="huile-modal-tab-active" className="absolute bottom-0 left-0 right-0 h-0.5 bg-gold" />
                        )}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="min-h-[160px]">
                    <AnimatePresence mode="wait">
                      {activeTab === 'description' && (
                        <motion.div key="desc" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }} className="space-y-4 max-w-3xl">
                      ...
                      {activeTab === 'notes' && (
                        <motion.div key="notes" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }} className="max-w-2xl">
                      ...
                      {activeTab === 'details' && (
                        <motion.div key="details" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
                          <table className="max-w-2xl w-full border-collapse">
                            <tbody>
                              {product.code_reference && (<tr className="border-b border-foreground/10"><td className="w-1/3 py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Reference' : 'Code Réf.'}</td><td className="py-3.5 font-medium text-foreground font-mono">{product.code_reference}</td></tr>)}
                              {product.intensite && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Intensity' : 'Intensité'}</td><td className="py-3.5 font-medium text-foreground capitalize">{product.intensite}</td></tr>)}
                              {product.genre_cible && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Target' : 'Cible'}</td><td className="py-3.5 font-medium text-foreground capitalize">{product.genre_cible}</td></tr>)}
                              {product.famille_olfactive && product.famille_olfactive.length > 0 && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Olfactive Family' : 'Famille olfactive'}</td><td className="py-3.5 font-medium text-foreground">{product.famille_olfactive.join(', ')}</td></tr>)}
                              <tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">Type</td><td className="py-3.5 font-medium text-foreground">{isEn ? 'Pure Essential Oil' : 'Huile Essentielle Pure'}</td></tr>
                              <tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Brand' : 'Marque'}</td><td className="py-3.5 font-medium text-foreground">{product.brand || 'Exclusif Collection'}</td></tr>
                              {product.origine_pays && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Origin' : 'Pays d\'origine'}</td><td className="py-3.5 font-medium text-foreground">{product.origine_pays}</td></tr>)}
                              {variants.length > 0 && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Available sizes' : 'Formats disponibles'}</td><td className="py-3.5 font-medium text-foreground">{variants.map(v => `${v.taille_ml}ml (${formatPrice(v.prix_actuel)})`).join(', ')}</td></tr>)}
                              {product.stock_total_ml != null && (<tr className="border-b border-foreground/10"><td className="py-3.5 text-xs uppercase tracking-widest text-foreground/40">{isEn ? 'Lab stock' : 'Stock laboratoire'}</td><td className="py-3.5 font-medium text-foreground font-mono">{product.stock_total_ml.toLocaleString('fr-FR')} ml</td></tr>)}
                            </tbody>
                          </table>
                        </motion.div>
                      )}
                      ...
                      {activeTab === 'usage' && (
                        <motion.div key="usage" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }} className="max-w-2xl">
                          <div className="space-y-4">
                            {[
                              {
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                      {relatedProducts.map(p => {
                        const isHuile = p.category === 'huile' || p.produits_finis !== undefined;
                        return (
                          <div
                            key={p.id}
                            className="cursor-pointer"
                            onClick={() => {
                              if (isHuile && onRelatedHuileClick) {
                                const pid = String(p.slug || p.id);
                                window.history.pushState({ modalProductId: pid }, '', `/shop/huile/${pid}`);
                                onRelatedHuileClick(p);
                              }
                            }}
                          >
                            {/* Simple card preview */}
                            <div className="rounded-2xl border border-foreground/10 bg-foreground/5 overflow-hidden hover:border-gold/30 transition-all">
                              {p.images?.[0] && (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={resolveImageUrl(p.images[0])} alt={p.name} className="w-full aspect-square object-cover" />
                              )}
                              <div className="p-3">
                                <p className="text-xs font-bold text-foreground truncate">{p.name}</p>
                                <p className="text-xs text-gold font-mono mt-1">{formatPrice(p.price)}</p>
                              </div>
                            </div>
                          </div>
                        );
