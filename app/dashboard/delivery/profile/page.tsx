  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-2xl font-bold text-foreground">{t('delivery_profile')}</h1>
        <button 
          onClick={() => logout()}
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-6 shadow-2xl flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <span className="text-[10px] font-bold text-gold bg-gold/10 px-2 py-0.5 rounded mt-1 inline-block uppercase tracking-wider">{t('certified_delivery')}</span>
        </div>
        <button 
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-foreground/40 uppercase tracking-widest px-1">{t('delivery_pro_info')}</h3>
        <div className="bg-white/5 rounded-2xl border border-white/10 shadow-2xl overflow-hidden divide-y divide-white/5">
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-bold text-foreground/40 uppercase">{t('email_pro')}</p>
          ...
          <div className="flex items-center justify-between px-5 py-4">
            ...
            <button 
              onClick={() => setShowEditModal(true)}
              className="text-xs text-gold font-medium">{t('edit')}</button>
          </div>
        </div>
      </div>
           ...
           <button className="w-full flex items-center justify-between px-5 py-4 hover:bg-white/5 transition-all group">
             <div className="flex items-center gap-3">
               ...
               <div className="text-left">
                 ...
                 <p className="text-[11px] text-foreground/40">{t('delivery_alerts')}</p>
               </div>
             </div>
             <ChevronRight size={16} className="text-foreground/20" />
           </button>
